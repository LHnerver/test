...
/**
  ...
  */
uint8_t SCA_Write_1(SCA_HANDLER* pSCA, uint8_t cmd, uint8_t TxData)
{
	uint8_t TxBuf[2];
	...
	return CAN_Transmit(pSCA, TxBuf, 2);
}
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t SCA_Write_2(SCA_HANDLER* pSCA, uint8_t cmd, float TxData)
{
	uint8_t TxBuf[3];
	int16_t temp;
	...
	return CAN_Transmit(pSCA, TxBuf, 3);
}
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t SCA_Write_3(SCA_HANDLER* pSCA, uint8_t cmd, float TxData)
{
	uint8_t TxBuf[5];
	int32_t temp;
	...
	if((cmd == W3_Velocity) || (cmd == W3_VelocityLimit))
		temp = TxData / pSCA->Velocity_Max * IQ24;
	else if((cmd == W3_Current)||(cmd == W3_CurrentLimit)||(cmd == W3_CurrentFilterLimitL)||(cmd == W3_CurrentFilterLimitH))
	...
	return CAN_Transmit(pSCA, TxBuf, 5);
}
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t SCA_Write_4(SCA_HANDLER* pSCA, uint8_t cmd)
{
	uint8_t TxBuf[1] ; 
	TxBuf[0]= cmd;
	return CAN_Transmit(pSCA, TxBuf, 1);
}
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t SCA_Write_5(SCA_HANDLER* pSCA, uint8_t cmd, uint8_t TxData)
{
	uint8_t TxBuf[6];
	...
	*/
	...
	return CAN_Transmit(pSCA, TxBuf, 6);
}

/**
  * @功	能	第6类写入命令，发送5byte，接收2byte
  * @参	数	pSCA：要操作的执行器句柄指针或地址
  *			cmd：操作指令
  *			TxData：发送数据
  * @返	回	SCA_NoError：发送成功
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t SCA_Write_6(SCA_HANDLER* pSCA, uint8_t cmd, uint32_t TxData)
{
	uint8_t TxBuf[5];
...
/**
  ...
  *			其他通信错误参见 SCA_Error 错误列表
  */
uint8_t SCA_Read(SCA_HANDLER* pSCA, uint8_t cmd)
{
	uint8_t TxBuf[1];
	TxBuf[0]= cmd;
	return CAN_Transmit(pSCA, TxBuf, 1);
...
uint8_t CAN_Transmit(SCA_HANDLER* pSCA, uint8_t* TxBuf, uint8_t TxLen)
{
	uint32_t waitime = 0;
...
/**
  ...
  * @返	回	无
  */
void R1dataProcess(SCA_HANDLER* pSCA, CAN_MSG* RxMsg)
{
	/* 将读取结果装载到接收地址中 */
	switch(RxMsg->data[0])
	{
...
/**
  ...
  * @返	回	无
  */
void R2dataProcess(SCA_HANDLER* pSCA, CAN_MSG* RxMsg)
{
	int16_t temp;
	float RxData;
	...
	{
		RxData = (float)((uint16_t)temp / IQ10);
	}
	else if (RxMsg->data[0] == R2_Error)
	{
		RxData = temp;
	}
	else
...
/**
  ...
  * @返	回	无
  */
void R3dataProcess(SCA_HANDLER* pSCA, CAN_MSG* RxMsg)
{
	int32_t temp;
	float RxData;
	...
	if((RxMsg->data[0] == R3_Velocity)||(RxMsg->data[0] == R3_VelocityLimit))
		RxData = (float)temp / IQ24 * pSCA->Velocity_Max;
	else if((RxMsg->data[0] == R3_Current)||(RxMsg->data[0] == R3_CurrentLimit)||
			(RxMsg->data[0] == R3_CurrentFilterLimitL)||(RxMsg->data[0] == R3_CurrentFilterLimitH))
	...
	switch(RxMsg->data[0])
	{
		...
		case R3_CurrentFilterLimitL:
			pSCA->Current_Filter_Limit_L = RxData;
			pSCA->paraCache.R_Current_Filter_Limit_L = Actr_Enable;
			break;
		
		case R3_CurrentFilterLimitH:
			pSCA->Current_Filter_Limit_H = RxData;
			pSCA->paraCache.R_Current_Filter_Limit_H = Actr_Enable;
			break;
...
/**
  ...
  * @返	回	无
  */
void R4dataProcess(SCA_HANDLER* pSCA, CAN_MSG* RxMsg)
{
	int32_t temp;	
	...
	pSCA->Velocity_Real = (float)temp / IQ30 * pSCA->Velocity_Max;
...
/**
  ...
  * @返	回	无
  */
void R5dataProcess(SCA_HANDLER* pSCA, CAN_MSG* RxMsg)
{
	/* 装填序列号 */
...
/**
  ...
  * @返	回	无
  */
void WriteDataProcess(SCA_HANDLER* pSCA, CAN_MSG* RxMsg)
{
	/* 写入成功，将缓存中的参数更新到句柄中 */
	if(RxMsg->data[1] == Actr_Enable)
	{
		...
		switch(RxMsg->data[0])
		{
			...
			case W3_CurrentFilterLimitL:
				pSCA->Current_Filter_Limit_L = pSCA->paraCache.Current_Filter_Limit_L;
				break;
			
			case W3_CurrentFilterLimitH:
				pSCA->Current_Filter_Limit_H = pSCA->paraCache.Current_Filter_Limit_H;
				break;
...
/**
  ...
  * @返	回	无
  */
void CAN_Dispatch(SCA_HANDLER* pSCA,CAN_MSG* RxMsg)
{
	/* 标记有数据更新 */
...
/**
  ...
  * @返	回	无
  */
void WarnBitAnaly(SCA_HANDLER* pSCA)
{
